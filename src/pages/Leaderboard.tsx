...
import { Trophy, Medal, Crown, TrendingUp, Zap, Flame, Calendar } from 'lucide-react';
        ...
        <div className="container mx-auto px-4">
          ...
          >
            ...
            <div className="inline-flex items-center gap-4 px-6 py-3 rounded-full bg-background/10 backdrop-blur-sm border border-background/20">
              ...
              <span className="text-2xl font-bold text-background">#{mockUser.rank}</span>
              <div className="w-px h-6 bg-background/20" />
              <div className="flex items-center gap-1 text-primary">
                <Zap className="w-5 h-5" />
                <span className="font-semibold text-background">{mockUser.xp.toLocaleString()} XP</span>
              </div>
            </div>
          </motion.div>
        </div>
          ...
          <div className="flex items-end justify-center gap-4 md:gap-8">
            ...
            >
              ...
              <div className="bg-card rounded-t-2xl p-4 md:p-6 text-center min-w-[120px] md:min-w-[160px] h-32 border border-border">
                <p className="font-semibold text-card-foreground truncate">{topThree[1]?.user.name}</p>
                <p className="text-sm text-muted-foreground">Level {topThree[1]?.user.level}</p>
            ...
            >
              ...
              <div className="bg-gradient-to-b from-warning/10 to-card rounded-t-2xl p-4 md:p-6 text-center min-w-[140px] md:min-w-[180px] h-40 border border-warning/30">
                <p className="font-bold text-lg text-card-foreground truncate">{topThree[0]?.user.name}</p>
                <p className="text-sm text-muted-foreground">Level {topThree[0]?.user.level}</p>
                ...
                <div className="flex items-center justify-center gap-1 mt-1 text-accent">
                  ...
                  <span className="text-sm">{topThree[0]?.user.streak} day streak</span>
                </div>
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="bg-card rounded-t-2xl p-4 md:p-6 text-center min-w-[120px] md:min-w-[160px] h-28 border border-border">
                <p className="font-semibold text-card-foreground truncate">{topThree[2]?.user.name}</p>
                <p className="text-sm text-muted-foreground">Level {topThree[2]?.user.level}</p>
                <div className="flex items-center justify-center gap-1 mt-2 text-primary">
                  <Zap className="w-4 h-4" />
